...
if (!function_exists('shortcode_recent_posts')) {
	...
	function shortcode_recent_posts($atts, $content = null) {		
		...
		foreach($latest as $post) {
				...
				$ul = get_post_thumbnail_id($post->ID);
				...
				$image = vt_resize( $thumb,'' , 200, 120, true, 100 );
				...
				$output .= '<figure class="thumbnail featured-thumbnail large">';
					...
					$output .= 'ssss<img src="' . $image[url] . '" width="'.$image[width].'" height="'.$image[height].'" alt="' . get_the_title($post->ID) .'" />';
					...
					$output .= '<script type="text/javascript">
						jQuery(document).ready(function(){
							...
							}, [
							{
								...
								ready: function () {jQuery(this).jPlayer("setMedia", {'. $audio_format .' : "'. stripslashes(htmlspecialchars_decode($audio_url)) .'", poster: "'. $image .'"});
							},
				...
				//Video
					...
					if(has_post_thumbnail()) {
						...
						$image = vt_resize( $thumb,'' , 770, 380, true, 100 );
					}
						...
						$output .= '<script type="text/javascript">
							jQuery(document).ready(function(){
								...
								}, [
								{
									...
									playlistOptions: {enableRemoveControls: false},
									ready: function () {jQuery(this).jPlayer("setMedia", {m4v : "'. stripslashes(htmlspecialchars_decode($m4v_url)) .'", ogv : "'. stripslashes(htmlspecialchars_decode($ogv_url)) .'",});
								},
								swfPath: "'. $template_url .'/flash",
				...
				if ($thumb == 'true') {
						if ( has_post_thumbnail($post->ID) ){
								...
								$output .= '<img src="' . $image[url] . '" width="'.$image[width].'" height="'.$image[height].'" alt="' . get_the_title($post->ID) .'" />';
								$output .= '</a></figure>';
						}
					}
...
$random = hs_gener_random(10);		
		...
		foreach($testi as $post){
				...
				$ul = get_post_thumbnail_id($post->ID);
				if ($thumb == 'true') {
				$image = vt_resize( $ul,'' , 280, 240, true, 100 );
                }
...
if ($thumb == 'true') {
								...
								if ( has_post_thumbnail($post->ID) ){
									$output .= '<div class="testi-image" style="margin:0 auto; width:'.$image['width'].'px; height:'.$image['height'].'px;">';
									$output .= '<img src="'.$image['url'].'" width="'.$image['width'].'" height="'.$image['height'].'" />';
									$output .= '</div>';
								}
							}

					$output .= '<span class="testi-meta">';
							if($testiname) { 
