...
class OptionsFramework_Backup {
	...
	<div id="optionsframework-metabox" class="metabox-holder">
		<div id="optionsframework" class="postbox">
			<div class="wrap">
				...
				<h3><?php _e( 'Import', HS_CURRENT_THEME ); ?></h3>
	...
	function callImportTheme () { ?>
		<div class="import-options-settings">
			<h4 class="head"><?php _e( '<span>Step <i>1</i></span>Theme Options Import', HS_CURRENT_THEME ); ?></h4>			
			<p class="text-style"><?php _e( 'Select the file that contains Theme Options (e.g. <b>options.json</b> )', HS_CURRENT_THEME ); ?></p>
			...
			<form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=options-framework-import'); ?>" id="optionsForm">
				...
				<input type="submit" class="button-primary" value="<?php _e( 'Upload File and Import', HS_CURRENT_THEME ); ?>" disabled="disabled">
			</form>
		</div>
	<?php 
	}
	...
	<div id="optionsframework-metabox" class="metabox-holder">
		<div id="optionsframework" class="postbox">
			<div class="wrap">
				<!--?php echo get_screen_icon( $screen = 'import-export' ); ?-->	
				<h3><?php _e( 'Export', HS_CURRENT_THEME ); ?></h3>
...
<?php
	...
	function callExportTheme () { ?>
		<h4 class="head"><?php _e( '<span>Step <i>1</i></span>Theme Options Export', HS_CURRENT_THEME ); ?></h4>
		...
		<form method="post" action="" id="theme-options-export">
			...
			<input type="submit" class="button-primary fnone" value="<?php _e( 'Next &raquo;', HS_CURRENT_THEME ); ?>" />
		</form>
	<?php }
	...
	/*function admin_screen_help ( $contextual_help, $screen_id, $screen ) {
		...
		$contextual_help =
		  '<h3>' . __( 'Welcome to the OptionsFramework Backup Manager.', HS_CURRENT_THEME ) . '</h3>';		
		} // End IF Statement
	...
	*/
	...
	function admin_notices () {
		...
		if ( isset( $_GET['error'] ) && $_GET['error'] == 'true' ) {
			echo '<div id="message" class="error"><p>' . __( 'There was a problem importing your settings. Please Try again.', HS_CURRENT_THEME ) . '</p></div>';
		} else if ( isset( $_GET['error-export'] ) && $_GET['error-export'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'There was a problem exporting your settings. Please Try again.', HS_CURRENT_THEME ) . '</p></div>';
		} else if ( isset( $_GET['invalid'] ) && $_GET['invalid'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'The import file you\'ve provided is invalid. Please try again.', HS_CURRENT_THEME ) . '</p></div>';
		} // End IF Statement
