...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function admin_init() {
			...
			if ( isset( $_REQUEST['hercules_tab'] ) && 'plugin-information' == $_REQUEST['hercules_tab'] ) {
				require_once ABSPATH . 'wp-admin/includes/plugin-install.php'; // Need for install_plugin_information()

				wp_enqueue_style( 'plugin-install' );

				global $hercules_tab, $hercules_body_id;
				$hercules_body_id = $hercules_tab = 'plugin-information';
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<?php if ( isset( $this->message ) ) _e( wp_kses_post( $this->message ), HS_CURRENT_THEME ); ?>
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', HS_CURRENT_THEME ) . '</a></p>';
				...
				foreach ( $this->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( $this->strings['return'], HS_CURRENT_THEME ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', HS_CURRENT_THEME ) . '">' . __( 'Return to the Dashboard', HS_CURRENT_THEME ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( __( 'The following plugin was activated successfully: %s.', HS_CURRENT_THEME ), '<strong>' . $plugin['name'] . '</strong>' );
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						foreach ( $plugin_groups as $plugin_group_single_name ) {
							...
							elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
								$url = add_query_arg(
									array(
										'hercules_tab'       => 'plugin-information',
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', HS_CURRENT_THEME ) . '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', HS_CURRENT_THEME );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', HS_CURRENT_THEME );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', HS_CURRENT_THEME );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', HS_CURRENT_THEME );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', HS_CURRENT_THEME ) : __( 'Recommended', HS_CURRENT_THEME );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['hercules_status'] = sprintf( '%1$s', __( 'Not Installed', HS_CURRENT_THEME ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['hercules_status'] = sprintf( '%1$s', __( 'Installed But Not Activated', HS_CURRENT_THEME ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', HS_CURRENT_THEME ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', HS_CURRENT_THEME ),
				'source' => __( 'Source', HS_CURRENT_THEME ),
				'type'   => __( 'Type', HS_CURRENT_THEME ),
				'hercules_status' => __( 'Status', HS_CURRENT_THEME )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', HS_CURRENT_THEME ),
				'tgmpa-bulk-activate' => __( 'Activate', HS_CURRENT_THEME ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, HS_CURRENT_THEME ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			/**
	 		 ...
	 		 */
			public function run( $hs_options ) {
				...
				$hs_options = wp_parse_args( $hs_options, $defaults );
				extract( $hs_options );
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', HS_CURRENT_THEME );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', HS_CURRENT_THEME );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', HS_CURRENT_THEME );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', HS_CURRENT_THEME );
				$this->strings['process_failed']      = __( 'Plugin install failed.', HS_CURRENT_THEME );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', HS_CURRENT_THEME );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', HS_CURRENT_THEME );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', HS_CURRENT_THEME );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', HS_CURRENT_THEME );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', HS_CURRENT_THEME ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', HS_CURRENT_THEME ) . '</span><span class="hidden">' . __( 'Hide Details', HS_CURRENT_THEME ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', HS_CURRENT_THEME );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', HS_CURRENT_THEME );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', HS_CURRENT_THEME );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', HS_CURRENT_THEME );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', HS_CURRENT_THEME );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', HS_CURRENT_THEME ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', HS_CURRENT_THEME ) . '</span><span class="hidden">' . __( 'Hide Details', HS_CURRENT_THEME ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', HS_CURRENT_THEME );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', HS_CURRENT_THEME );
				}
			...
			public function bulk_footer() {
				...
				foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( TGM_Plugin_Activation::$instance->parent_url_slug ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . __( TGM_Plugin_Activation::$instance->strings['return'], HS_CURRENT_THEME ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', HS_CURRENT_THEME ) . '">' . __( 'Return to the Dashboard', HS_CURRENT_THEME ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
